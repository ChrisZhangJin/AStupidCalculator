...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        # if input.shape[0] < hidden.shape[1]:
        #     # batch not match!
        #     gap = input[-1].unsqueeze(0).repeat((hidden.shape[1]-input.shape[0]), 1, 1)
        #     input = torch.cat([input, gap], dim=0)
        #     print(input.shape)
...
class Trainer:
    ...
    def train_epoch(self):
        ...
        for input, target in self.dataloader:
            ...
            # embedded_target = self.embedding_layer(target_tensor)
    ...
    def save_checkpoint(self, epoch, path='.'):
        ...
        pfilepath = os.path.join(path, '{0}_epoch{1}_acc{2}_params.txt'.format(filename, epoch, int(self.latest_acc*100)))
